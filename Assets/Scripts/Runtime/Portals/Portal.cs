...
public class Portal : MonoBehaviour
{
    ...
    {
        // 清除老位置的traveller。
        // 如果在LateUpdate中处理传送，则必须执行，否则同一物理帧中传送门位置改变后，
        // 可能导致传送门新位置与当前traveller的夹角超过180度而在同一帧的LateUpdate中被错误传送
        foreach (var traveller in trackedTravellers)
        {
            ChangeWallCollider(traveller, false);

            traveller.ExitPortalThreshold();
        }
        trackedTravellers.Clear();

        // 检测新位置的traveller，也可等下一帧的OnTriggerEnter自行检测触发，影响不大
        //var boxCollider = GetComponent<BoxCollider>();
        //var colliderBounds = boxCollider.bounds;
        //Collider[] newColliders = Physics.OverlapBox(colliderBounds.center, colliderBounds.extents, transform.rotation);
        //foreach (var collider in newColliders)
        //{
        //    if (collider == boxCollider) continue; // 排除自身的碰撞器

        //    PortalTraveller traveller = collider.GetComponent<PortalTraveller>();
        //    if (traveller != null)
        //    {
    ...
    /// <summary>
    /// 传送过程虽然属于逻辑更新，但由于逻辑帧率通常较低，会导致视觉不连贯；
    /// 改为LateUpdate中更新可保证视觉连贯，但需要处理trigger对象的转移
    /// </summary>
    void LateUpdate()
    {
        HandleTravellers();
    ...
    void HandleTravellers()
    {
        ...
        for (int i = 0; i < trackedTravellers.Count; i++)
        {
            ...
            if (portalSide != portalSideOld)
            {
                ...
                // 注意ChangeWallCollider应该先调用，避免两个传送门在同一个Wall上时碰撞错误的先关闭再开启
                ChangeWallCollider(traveller, false);
    ...
    void HandleClipping()
    {
        const float hideDst = -500;
        const float showDst = 500;
    ...
    void OnTravellerEnterPortal(PortalTraveller traveller)
    {
        if (!trackedTravellers.Contains(traveller))
        {
            ...
            ChangeWallCollider(traveller, true);
        }
    }
    ...
    /// 设置传送门所靠墙对传送物体的碰撞
    /// </summary>
    void ChangeWallCollider(PortalTraveller traveller, bool ignore)
    {
        if (traveller != null && wallCollider != null)
        {
            var colliders = traveller.GetComponentsInChildren<Collider>();
            foreach (var collider in colliders)
            {
                Physics.IgnoreCollision(wallCollider, collider, ignore);
            }
        }
    }
